...
The last show was pretty wonderful, but I always want to improve things. Something that's been bugging me in my video setup is, that, it was a mess. I mean, I had HDMI cables here, SDI there, some RTSP streams, and even NDI feeds. Talk about a tech salad 🥗!
...
As I mentioned, the issue had always been the budget 💰, I was at the time an apprentice in a different field with no contacts or knowledge in the AV production world. Video isn't exactly the cheapest part of a live production, and the technology evolves rapidly, the quality always needs to increase.
...
I started digging into different options and, there are a few contenders:
  ...
  - **NDI:** This system is pretty slick. But there is a catch, you either need fancy cameras that speak NDI right out of the box, or you have to shell out for converters that will set me back at least 300 euros a pop 🙁. Not to mention that if you want to get low latency, you need special mixers.
  - **RTSP:** Now, this one's tempting. It's wonderful, very popular, but it's got a bit of a lag issue. And, you still need converter boxes. Although they are a bit cheaper.
...
But wait, it gets better! SDI's got a neat trick up its sleeve - you can easily convert it to fiber ⚪. Like effortlessly convert it!
...
This setup would be like the Swiss Army knife 🔪 of video transport. I'm thinking of something like 2 fibers for 10Gb Ethernet 🌐, at least 8 for video feeds 📹, and 2 spares that I'll probably use for audio in the future. 
...
Unfortunately, I couldn't find a cheaper alternative, even for lower data rates. I was on the lookout for more budget-friendly alternatives, and purely by chance, I stumbled upon this post by twi_kingyo.
...
Most of them don't need to be dynamically adjusted, and some can be ignored outright. As long as the module gets power, has the correct rate and has its transmitter is enabled, it's fine.
...
What's even better is that they are cheap. The SFP+ transceivers that I have are the [DELL FTLX8571D3BCL-FC](https://www.dell.com/en-us/shop/c2g-finisar-ftlx8571d3bcl-compatible-10gbase-sr-mmf-sfp-transceiver-module-taa-sfp-transceiver-module-10-gigabi/apd/a8568835/pc-accessories). I got them for around 8eur, but you can get them cheaper, here is an eBay listing for ten modules for 70eur. 

{{< og "https://www.ebay.fr/itm/116299329366" >}}

Now, there are special ones for video that can handle weirder and faster signals with better signal integrity (12G-SDI for example) but from my testing, 3G SDI works just fine on these modules.
...
After this extensive research, It would seem that all that I really need is a cable equalizer for RX and a cable driver for TX to convert the 75Ohm single-ended and 100Ohm differential pairs 🤷‍♂️.
...
### PCB
This is the first prototype with the appropriate dimensions, my plan was to fit everything into a 1U case (fiber splitter, converter modules, …). This means it has to be at most 45 mm tall but decided that my target would be 35 mm 📏. This would leave enought space for the chassis, mounting & cables.
...
On the other hand, the datasheet of the `LMH0344` talks way more about 3G-SDI. Alongside that, it also has a `Mute` pin and `Cable/Carrier detect` signalling pin.
...
Both of these issues can be solved by adding 0ohm resistors that may or may not be placed during assembly.
...
I wish I had a video to show you but as I'll explain later, I don't have any V3 board that are fully populated anymore
...
While doing some tests, I realized that the very first prototype used the `LMH0397` which, while uni-directional, did include a reclocker ⏱.
...
For fun, I also tried to chain every piece of fiber and coax I had ⛓️. This came to a total of 95m of fiber 30m of coax if this test completed successfully, it would mean that the signal would have traveled 220m 🤔. And to my surprise, **IT WORKED 😮🥳**
...
One thing that I also wanted to change was the connector. The one I was using were 50Ohm SMA connectors, which is both the wrong impedance and connector for SDI.
...
Instead, I simply used the footprint of the [034-1030-12G](https://www.amphenolrf.com/034-1030-12g.html) from Amphenol which is a 12G 75Ohm right angle HD-BNC connector. This means that these boards are technically compatible with a connector, but there's no way in hell I'm going to use it.
...
Here is a photo of it soldered, unfortunately the rest of the pigtails and SFP cages didn't arrive yet:
...
When I designed the PCBs, I originally wanted to use standoffs to carry current. But when assembly time came, I realized that that would be a nightmare to disassemble 😫. So to solve this, I decided to create a custom 3D-printed mount that could house eight of these converters (with only 6 slots populated, two reclocked bidirectional and four uni-directional normal ones). 
...
As for the price, without including the time it took to design and assemble them, I would say that I reached my price target by a small enought margin 🤑.
But what I do realize is that if I had to pay someone to design this, it wouldn't be economical, at all 💸. And suddenly the 155eur price of the Blackmagic converter (which can do 12G-SDI) seems much more reasonable, I would even say cheap!
...
Looking back, the most important takeaways are:
  ...
  - **Iterative design** isn't something I generally do, mostly due to budget constrains 💵. Except that in this instance, each prototype taught me something new, leading to better performance in the next iteration 🔄. I'll definitively do it more
